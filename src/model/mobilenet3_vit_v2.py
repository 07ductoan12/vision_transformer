...
class LinearSelfAttention(nn.Module):
    def __init__(self, embed_dim, attn_dropout=0):
        ...
        self.out_proj = conv_2d(
            embed_dim, embed_dim, kernel_size=1, bias=True, norm=False, act=False
        )
    ...
    def __init__(self, inp, attn_dim, ffn_multiplier, attn_blocks, patch_size):
        ...
        for i in range(attn_blocks):
            ...
            self.global_rep.add_module(
                f"LinearAttnFFN_{i}", LinearAttnFFN(attn_dim, ffn_dim)
            )
        self.global_rep.add_module(
            "LayerNorm2D",
            nn.GroupNorm(num_channels=attn_dim, eps=1e-5, affine=True, num_groups=1),
        )

        self.conv_proj = conv_2d(
            2 * attn_dim, inp, kernel_size=1, stride=1, padding=0, act=False
        )
...
class MobileViTv3_v2(nn.Module):
    def __init__(self, image_size, width_multiplier, num_classes, patch_size=(2, 2)):
        ...
        self.layer_3 = nn.Sequential(
            ...
            MobileViTBlockv3_v2(
                channels[3], attn_dim[0], ffn_multiplier, 2, patch_size=patch_size
            ),
        )
        self.layer_4 = nn.Sequential(
            ...
            MobileViTBlockv3_v2(
                channels[4], attn_dim[1], ffn_multiplier, 4, patch_size=patch_size
            ),
        )
        self.layer_5 = nn.Sequential(
            ...
            MobileViTBlockv3_v2(
                channels[5], attn_dim[2], ffn_multiplier, 3, patch_size=patch_size
            ),
        )
